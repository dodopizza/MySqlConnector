		...
		public static Task<DbDataReader> ExecuteReaderAsync(IReadOnlyList<IMySqlCommand> commands, ICommandPayloadCreator payloadCreator, CommandBehavior behavior, IOBehavior ioBehavior, CancellationToken cancellationToken) =>
			DiagnosticListener.WithDiagnosticForCommand<DbDataReader>(commands, async () =>
			{
				...
				if (Log.IsDebugEnabled())
					Log.Debug("Session{0} ExecuteReader {1} CommandCount: {2}", connection.Session.Id, ioBehavior,
						commands.Count);
				...
				using var registration =
					command.CancellableCommand
						.RegisterCancel(cancellationToken); // lgtm[cs/useless-assignment-to-local]
				connection.Session.StartQuerying(command.CancellableCommand);
				command.SetLastInsertedId(-1);
				try
				{
					await connection.Session.SendAsync(payload, ioBehavior, CancellationToken.None)
						.ConfigureAwait(false);
					return await MySqlDataReader.CreateAsync(commandListPosition, payloadCreator, cachedProcedures,
						command, behavior, ioBehavior, cancellationToken).ConfigureAwait(false);
				}
