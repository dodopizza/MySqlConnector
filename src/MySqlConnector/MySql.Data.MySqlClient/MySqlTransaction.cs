...
namespace MySql.Data.MySqlClient
{
	public sealed class MySqlTransaction : DbTransaction
	{
		...
		private Task CommitAsync(IOBehavior ioBehavior, CancellationToken cancellationToken) =>
			s_diagnosticListener.WithDiagnosticForCommit(IsolationLevel, Connection, async () =>
			{
				VerifyNotDisposed();
		...
		private Task RollbackAsync(IOBehavior ioBehavior, CancellationToken cancellationToken) =>
			s_diagnosticListener.WithDiagnosticForRollback(IsolationLevel, Connection, async () =>
			{
				VerifyNotDisposed();
				if (Connection is null)
					throw new InvalidOperationException("Already committed or rolled back.");
...
#endif
		{
			if (Connection?.CurrentTransaction == this)
			{
				...
				{
					await s_diagnosticListener.WithDiagnosticForRollback(IsolationLevel, Connection, async () =>
					{
						using (var cmd = new MySqlCommand("rollback", Connection, this))
							await cmd.ExecuteNonQueryAsync(ioBehavior, cancellationToken).ConfigureAwait(false);
					});
